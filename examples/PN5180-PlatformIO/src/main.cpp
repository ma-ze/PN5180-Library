...
#define PN5180_NSS_SECONDARY    4 
#define PN5180_BUSY_SECONDARY   3
#define PN5180_RST_SECONDARY    2
...
void setup() {
  ...
  bool didReset = nfc.reset();
  if (!didReset) {
    Serial.println("RFID reader not present, moving to secondary");
    nfc = PN5180ISO15693(PN5180_NSS_SECONDARY, PN5180_BUSY_SECONDARY, PN5180_RST_SECONDARY);
    nfc.begin();
    nfc.reset();
  }
...
void loop() {
  if (errorFlag) {
    bool success;
    uint32_t irqStatus = nfc.getIRQStatus(success);
